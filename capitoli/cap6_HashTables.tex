...
La tecnica di \emph{separate chaining} è efficiente e facile da implementare, ma richiede inevitabilmente spazio aggiuntivo per le strutture dati ausiliarie. Se si vuole evitare di utilizzare spazio aggiuntivo per gestire le collisioni, l'unico approccio possibile è quello di cercare uno slot alternativo, sempre all'interno del nostro array, in cui memorizzare l'elemento che causa la collisione. 
...
\begin{itemize}
    ...
    \item Una scelta comune per \(d(k)\) è \(d(k) = q - (k \mod q)\), dove \(q\) è un numero primo più piccolo di \(N\).
\end{itemize}
