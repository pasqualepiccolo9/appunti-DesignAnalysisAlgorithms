...
\begin{itemize}[nosep]
    ...
    \item Un \textbf{sottografo} $H$ di un grafo $G = (V,E)$ è un grafo $H = (V', E')$ tale che $V' \subseteq V$ e $E' \subseteq E$. In altre parole, un sottografo è ottenuto rimuovendo vertici e/o archi da $G$. 
    \item Uno \textbf{spanning subgraph} (la traduzione corretta è "sottografo ricoprente") di un grafo $G = (V,E)$ è un sottografo $H = (V', E')$ tale che $V' = V$. In altre parole, uno spanning subgraph contiene tutti i vertici di $G$, ma potrebbe non contenere tutti gli archi di $G$. 
...
3.      \(D[s] = 0\)
4.      \textbf{for each} vertex \(v \neq s\) \textbf{do}
5.          \(D[v] = +\infty\)
6.      Initialize \(T\) = empty set
7.      Initialize a priority queue \(Q\) with an entry (\(D[v], (v, \text{None})\))  
             \textbf{for each} vertex \(v\), where \(D[v]\) is the key in the priority  
             queue, and (\(v, \text{None})\) is the associated value.
8.      \textbf{while} \(Q\) is not empty \textbf{do}
9.          (\(u, e\)) = \text{value returned by } \(Q\).\text{remove\_min}()
10.         Connect vertex \(u\) to \(T\) using edge \(e\).
11.         \textbf{for each} edge \(e' = (u, v)\) such that \(v\) in \(Q\) \textbf{do}
12.             # check if edge \((u, v)\) better connects \(v\) to \(T\)
13.             \textbf{if} \(w(u, v) < D[v]\) \textbf{then}
14.                 \(D[v] = w(u, v)\)
15.                 Change the key of vertex \(v\) in \(Q\) to \(D[v]\).
16.                 Change the value of vertex \(v\) in \(Q\) to (\(v, e'\)).
17.     \textbf{return} the tree \(T\)     
\end{alltt}
\hrule 
\vspace{1\baselineskip}
...
def MST_PrimJarnik(g):
    ...
    while not pq.is_empty( ):
        ...
        if edge is not None:   # if it's not the first node
...
Per implementare efficientemente questo approccio, sono necessarie:
...
\begin{itemize}[nosep]
    \item \texttt{makeSet(u)}: crea un nuovo insieme contenente solo il vertice $u$. $(O(1))$
    \item \texttt{find(u)}: restituisce l'identificativo dell'insieme a cui appartiene $u$. $(O(1))$
    \item \texttt{union(A, B)}: unisce i due insiemi $A$ e $B$ in un unico insieme, aggiornando i riferimenti. \\ $O(min (|A|, |B|))$
\end{itemize}
...
2.      \textbf{for each} vertex \(v\) in \(G\) \textbf{do}
...
6.      \textbf{while} \(T\) has fewer than \(n - 1\) edges \textbf{do}
7.          \((u, v)\) = \text{value returned by } \(Q\).\text{remove\_min}()
8.          \textbf{Let} \(C(u)\) be the cluster containing \(u\)
9.          \textbf{Let} \(C(v)\) be the cluster containing \(v\)
10.         \textbf{if} \(C(u) \neq C(v)\) \textbf{then}
11.             Add edge \((u, v)\) to \(T\).
12.             Merge \(C(u)\) and \(C(v)\) into one cluster.
13.     \textbf{return} tree \(T\)
\end{alltt}
\hrule 
\vspace{1\baselineskip}
...
\begin{lstlisting}[
    language=Python,
    caption={Implementazione Python dell'algoritmo di Kruskal. Da notare che questa implementazione restituisce la variabile tree la quale conterrà esattamente $V - k$ archi, dove $V$ è il numero di vertici e $k$ è il numero di componenti connesse. Nel caso di un grafo connesso, $k=1$ e quindi la lista conterrà $V-1$ archi, come previsto per un MST.},
    captionpos=b,
    label={lst:Kruskal},
]
def MST_Kruskal(g):
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Strutture Dati:} Prim utilizza una coda di priorità per gestire i vertici, mentre Kruskal utilizza una coda di priorità per gli archi e una struttura per insiemi disgiunti (Partition) per gestire i cluster di vertici.
...
4.      \textbf{while} \(Q\) is not empty \textbf{do}
5.          \((u, v)\) = \text{value returned by } \(Q\).\text{remove\_max}()
6.          Remove edge \((u, v)\) from \(T\)
7.          \textbf{if} there is no path between \(u\) and \(v\) in the graph \((V, T)\) \textbf{then}
8.              Add edge \((u, v)\) back to \(T\)  \{(u, v) is a bridge\}
9.      \textbf{return} tree \(T\)
\end{alltt}
\hrule 
\vspace{1\baselineskip}
