...
\begin{itemize}
    ...
    \item \textbf{Archi in avanti (\emph{forward edges}):} Per ogni arco $e = (u, v) \in E$ tale che $f(e) < c_e$, includiamo in $G_f$ l'arco $e' = (u, v)$ con capacità residua: 
    $$f(e) < c_e \implies c_f(e') = c_e - f(e)$$ 
    \emph{Spiegazione:} Questi archi rappresentano la disponibilità residua del canale. Indicano che è possibile spingere ulteriori $c_e - f(e)$ unità di flusso da $u$ a $v$ senza violare la capacità originale.
    \item \textbf{Archi all'indietro (\emph{backward edges}):} Per ogni arco $e = (u, v) \in E$ tale che $f(e) > 0$, includiamo in $G_f$ l'arco inverso $e' = (v, u)$ con capacità residua: 
    $$f(e) > 0 \implies c_f(e') = f(e)$$
    \emph{Spiegazione:} Questi archi rappresentano la possibilità di "annullare" fino a $f(e)$ unità di flusso precedentemente inviate, spingendo flusso all'indietro.
\end{itemize}
Da notare che, per come sono costruiti gli archi del grafo residuo, questi sono tutti strettamente positivi: $c_f(e') > 0$ per ogni arco $e'$ in $G_f$.
...
Ci riferiamo alla capacità di un arco nel grafo residuo come \textbf{capacità residua}, per distinguere dalla capacità dell'arco corrispondente nella rete di flusso originale $G$.
...
Di seguito definiamo l'operazione \texttt{augment(P, f)}, che produce un nuovo flusso $f'$ in $G$.

\vspace{1\baselineskip}
\hrule
\begin{alltt}
Input: \(P\) è un cammino semplice \(s\)-\(t\) in \(G\sb{f}\)
        \(f\) è un flusso in \(G\)
Output: \(f'\) è un flusso in \(G\) ottenuto aumentando \(f\) lungo \(P\)
1.  Algorithm augment(\(P, f\)):
2.      \textbf{let} \(b = \text{bottleneck}(P, f)\)
3.      \textbf{for each} edge \(e = (u, v)\) in \(P\) \textbf{do}  # \(P\) composto da archi di \(G\sb{f}\)
4.          \textbf{if} \(e\) is a forward edge \textbf{then}
5.              \(f'(e) = f(e) + b\)
6.          \textbf{else}
7.              \(f'(e) = f(e) - b\)
8.      \textbf{return} \(f'\)  
...
Il risultato di \texttt{augment(P, f)} è un nuovo flusso $f'$ in $G$, ottenuto aumentando e diminuendo i valori di flusso sugli archi di $P$. 

\paragraph{Proposizione}
Sia $G = (V, E)$ una rete di flusso, e sia $f$ un flusso in $G$. Sia $P$ un cammino semplice $s-t$ in $G_f$. Allora l'output $f'$ di \texttt{augment(P, f)} è un flusso in $G$.
...
\begin{enumerate}
    \item Costruiamo il grafo residuo $G_f$ (a partire da $f$ flusso massimo, calcolato con Max-Flow).
