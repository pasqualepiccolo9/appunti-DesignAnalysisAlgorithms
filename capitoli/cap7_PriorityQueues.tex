...
\begin{itemize}
    ...
    \item Se il nodo in cui si è giunti è il figlio sinistro del suo genitore, vai al nodo fratello (cioè il figlio destro del genitore).
...
Il metodo \texttt{remove\_min()} rimuove e restituisce l'elemento con la chiave minima dalla Priority Queue. In un Heap, l'elemento con la chiave minima si trova sempre nella radice dell'albero. Per mantenere la proprietà di albero binario completo, dobbiamo sostituire la radice con l'ultimo nodo dell'Heap (cioè il nodo più a destra nell'ultimo livello dell'albero) e poi rimuovere l'ultimo nodo.

Dopo aver rimosso il nodo, è fondamentale aggiornare il riferimento all'ultimo nodo dell'Heap affinché punti al predecessore del nodo appena eliminato (secondo l'ordine di livello). La posizione del nuovo ultimo nodo può essere trovata in un tempo $O(\log n)$ a partire dal riferimento all'ultimo nodo rimosso:
\begin{itemize}
    \item Si parte dalla posizione del nodo che è stato appena rimosso (il vecchio ultimo nodo).
    \item Si risale l'albero fino alla radice o ad un nodo che è figlio destro del suo genitore.
    \item Se il nodo in cui si è giunti è il figlio destro del suo genitore, vai al nodo fratello (cioè il figlio sinistro del genitore).
    \item Infine, scendi sempre a destra fino a raggiungere un nodo foglia. Questo sarà il nuovo riferimento all'ultimo nodo.
\end{itemize}
