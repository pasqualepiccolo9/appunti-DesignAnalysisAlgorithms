...
L'algoritmo di DFS può essere utilizzato anche per attraversare grafi orientati, percorrendo ogni arco solo nella sua direzione.
\begin{itemize}
    ...
    \begin{itemize}[nosep]
        ...
        \item \textbf{back edges} (archi di ritorno): sono gli archi $(u, v)$ che collegano un vertice $u$ a un suo antenato\footnote{Nel contesto dell'albero di visita DFS, un vertice $v$ è definito \textbf{antenato} di $u$ se esiste un percorso composto esclusivamente da \textit{discovery edges} che collega $v$ a $u$. Viceversa, $u$ è \textbf{discendente} di $v$ se si trova nel sotto-albero radicato in $v$.} $v$ nell'albero DFS. La presenza di un arco di ritorno durante l'esecuzione della DFS indica l'esistenza di almeno un ciclo nel grafo.
...
\clearpage
\subsubsection{Searching Cycle}
Possiamo utilizzare DFS per cercare un ciclo semplice che contenga due vertici $v$ e $z$:
\begin{itemize}[nosep]
    \item Eseguiamo una DFS a partire da $v$;
    \item Utilizziamo uno stack $S$ per tenere traccia del cammino dal vertice di partenza $v$ al vertice corrente (e quindi degli archi di discovery attraversati);
    \item Non appena troviamo un arco di \emph{back} incidente su $z$, restituiamo tutti i vertici nello stack fino al vertice $v$.
\end{itemize}
...
10.                 cycleDFS(G, w, z) // Chiamata ricorsiva
