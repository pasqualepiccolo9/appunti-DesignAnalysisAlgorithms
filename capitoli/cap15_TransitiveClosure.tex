...
\paragraph{Definizione:} La \textbf{chiusura transitiva} (transitive closure) di un \textbf{grafo orientato} $\vec{G}=(V,E)$ è a sua volta un \emph{grafo orientato} $\vec{G}^*=(V^*,E^*)$ tale che:
\begin{itemize}[nosep]
    \item $V^* = V$
...
Output: The transitive closure \(G\sp{*}\) of \(G\)
1.  Algorithm FloydWarshall(\(G\))
2.       \(i = 1\)
3.       \textbf{for all} \(v\) in \(G\).\text{vertices}()       
4.           denote \(v\) as \(v\sb{i}\)              
5.           \(i = i + 1\)
6.       \(G\sb{0} = G\)                                  
7.       \textbf{for} \(k = 1\) \textbf{to} \(n\) \textbf{do}                // \(k\) è il vertice intermedio
8.           \(G\sb{k} = G\sb{k-1}\)                   // Copia stato precedente
9.           \textbf{for} \(i = 1\) \textbf{to} \(n\) (\(i \neq k\)) \textbf{do}   // Nodo sorgente
10.             \textbf{for} \(j = 1\) \textbf{to} \(n\) (\(j \neq i, k\)) \textbf{do} // Nodo destinazione
11.                 \textbf{if} \(G\sb{k-1}\).\text{areAdjacent}(\(v\sb{i}, v\sb{k}\)) \textbf{AND} 
12.                    \(G\sb{k-1}\).\text{areAdjacent}(\(v\sb{k}, v\sb{j}\))     
13.                    \textbf{if} NOT \(G\sb{k-1}\).\text{areAdjacent}(\(v\sb{i}, v\sb{j}\)) 
14.                        \(G\sb{k}\).\text{insertDirectedEdge}(\(v\sb{i}, v\sb{j}, k\)) 
15.      \textbf{return} \(G\sb{n}\)                               
\end{alltt}
\hrule 
\vspace{1\baselineskip}
