...
2. \textbf{While} (\(L\) is not empty \textbf{or} other finish condition is not met)
...
6. \textbf{Endwhile}
7. \textbf{Return} the resulting complete solution.
\end{alltt}
...
\begin{alltt}
Sort coins denominations by value: \(c[1] > c[2] > \dots > c[k]\) 
1. \(S = \{\}\)           // inizializza soluzione parziale (insieme vuoto)
2. \textbf{while} (\(x \neq 0\)) \{ // finché l'importo da cambiare non è zero
3.    \textbf{let} \(k\) be the largest integer such that \(c[k] \leq x\)
4.    \textbf{if} (\(k = 0\))
5.        \textbf{return} "no solution found"
6.    \(x = x - c[k]\)  // riduci l'importo rimanente
7.    \(S = \{S, c[k]\}\) // aggiungi \(c[k]\) alla soluzione
8. \}
9. \textbf{return} \(S\)         // restituisci la soluzione completa
\end{alltt}
...
\begin{alltt}
Sort intervals by starting time so that \(s[1] \leq s[2] \leq \dots \leq s[n]\)
1.  \(d = 0\)    // inizializza il numero di macchine
2.  \textbf{for} \(j = 1\) \textbf{to} \(n\) \{
3.      \textbf{if} (task \(j\) is compatible with some machine \(k\))
...
8.          \(d = d + 1\)
9.  \}
10. \textbf{return} \(d\)  // restituisci il numero di macchine utilizzate
\end{alltt}
...
\hrule
\begin{alltt}
Input:  Set \(S\) of items \(i\) with weight \(p\sb{i}\) and value \(v\sb{i}\) all positive
        Knapsack capacity \(P > 0\)
Output: Amount \(x\sb{i}\) of \(i\) that maximizes the total benefit without 
        exceeding in the capacity

1.  FractionalKnapsack(\(S, P\)):
2.      \textbf{for each} \(i\) \textbf{in} \(S\) \textbf{do}
3.          \(x\sb{i} = 0\)        // initially the knapsack is empty
4.          \(b\sb{i} = v\sb{i} / w\sb{i}\)          // the benefit of item \(i\)
5.      \(p = P\)                  // remaining capacity in knapsack (initially \(p = P\))
6.      \textbf{while} \(p > 0\) \textbf{do}
7.          remove from \(S\) an item of a maximal benefit      // greedy choice
8.          \(x\sb{i} = \min(p\sb{i}, p)\)    // can't carry more than \(p\)
9.          \(p = p - x\sb{i}\)             // update remaining capacity
...
\begin{itemize}[nosep]
    ...
    \item Un \textbf{prefix code} (codice prefisso) è un sistema di codifica binario in cui nessuna code-word è il prefisso di un'altra code-word. Questo garantisce che la decodifica sia univoca e non ambigua.
...
\vspace{1\baselineskip}
Ogni iterazione del ciclo while nell'algoritmo di Huffman può essere implementata in tempo $O(\log d)$ utilizzando una \emph{coda di priorità Q rappresentata con un heap}. Inoltre, ogni iterazione estrae due nodi da $Q$ e ne aggiunge uno, un processo che verrà ripetuto $d - 1$ volte prima che rimanga esattamente un nodo in $Q$. Pertanto, questo algoritmo funziona in tempo $O(n + d \log d)$, dove $n$ è la lunghezza della stringa $X$ e $d$ è il numero di caratteri distinti in $X$, e considerando che il calcolo delle frequenze richiede $O(n)$ tempo. 
...
4.      \textbf{for each} character \(c\) \textbf{in} \(X\) \textbf{do}
5.          Create a single-node binary tree \(T\) storing \(c\)
6.          Insert \(T\) into \(Q\) with key \(f(c)\)
7.      \textbf{while} len(\(Q\)) > 1 \textbf{do}
8.          (\(f\sb{1}\), \(T\sb{1}\)) = \(Q\).remove_min()
9.          (\(f\sb{2}\), \(T\sb{2}\)) = \(Q\).remove_min()
10.         Create a new binary tree \(T\) with left subtree \(T\sb{1}\) and 
            right subtree \(T\sb{2}\)
11.         Insert \(T\) into \(Q\) with key \(f\sb{1} + f\sb{2}\)
12.     (\(f, T\)) = \(Q\).remove_min()
