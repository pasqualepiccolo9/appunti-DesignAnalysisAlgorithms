...
\section{Neighbor Relation: Algoritmo Metropolis}
...
\clearpage
\section{Neighbor Relation: Simulated Annealing}
...
3.      \textbf{while} esiste un nodo \(v\) che migliora il cut value:
4.          \textbf{if} \(v\) in \(A\):  Sposta \(v\) da \(A\) a \(B\)
5.          \textbf{else}:       Sposta \(v\) da \(B\) a \(A\)
6.      \textbf{return} (\(A, B\))
\end{alltt}
\hrule
\vspace{1\baselineskip}
...
\clearpage
\section{Problema: Hopfield Neural Networks}
Le \textbf{Reti Neurali di Hopfield} (Hopfield Neural Networks) rappresentano un modello semplice di memoria associativa, in cui una vasta collezione di unità (neuroni) è connessa da una rete sottostante, e le unità vicine cercano di correlare i propri stati.
Il problema può essere modellato come segue:

\begin{itemize}
    \item \textbf{Input:} Un insieme $V$ di unità (nodi) e un insieme $E$ di coppie (archi).
    \item \textbf{Pesi:} Ogni coppia $e \in E$ possiede un peso intero $w_e$, che può essere positivo o negativo.
    \item \textbf{Soluzioni Ammissibili ($C$):} Una configurazione è un assegnamento di uno stato $s_u \in \{+1, -1\}$ ad ogni unità $u \in V$.
\end{itemize}

\subsection{Configurazioni Stabili}
...
\begin{itemize}
    \item Se $w_{uv} < 0$, allora $u$ e $v$ "vogliono" avere lo stesso stato (entrambi $+1$ o entrambi $-1$).
    \item Se $w_{uv} > 0$, allora $u$ e $v$ "vogliono" avere stati diversi.
\end{itemize}

\noindent
Formalizziamo questo concetto definendo la qualità dei singoli archi e nodi:
\begin{itemize}
    \item \textbf{Arco Buono (Good Edge):} Un arco $e=(u,v)$ si dice buono rispetto a una configurazione $S$ se il prodotto del peso per gli stati è negativo:
    $$ w_e s_u s_v < 0 $$
    Questa è una quantità negativa se si tratta di un arco buono, positiva altrimenti.
    \begin{itemize}
        \item Si ha un arco buono se $w_e < 0$ e $s_u = s_v$, oppure se $w_e > 0$ e $s_u \neq s_v$.
    \end{itemize}
    
    \item \textbf{Nodo Soddisfatto (Satisfied Node):} Un nodo $u$ è soddisfatto se il peso assoluto degli archi incidenti "buoni" è maggiore o uguale al peso degli archi incidenti "cattivi". Formalmente, la condizione è:
    $$
        \sum_{v: e=(u,v) \in E} w_{uv} s_u s_v \leq 0
    $$
    
    \item \textbf{Configurazione Stabile:} Una configurazione è stabile se \textbf{tutti} i nodi della rete sono soddisfatti.
\end{itemize}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.6\textwidth]{immagini/LocalSearch/node_satisfied.png}
    \caption{Esempio di nodo soddisfatto.}
    \label{fig:node_satisfied}
\end{figure}

\subsection{Algoritmo State-Flipping}
Per trovare una configurazione stabile, utilizziamo un algoritmo di ricerca locale chiamato \textbf{State-flipping Algorithm}.
La logica è estremamente semplice: partendo da una configurazione arbitraria, finché la rete non è stabile, si sceglie un nodo insoddisfatto e si inverte il suo stato.
...
3.      \textbf{while} esiste un nodo insoddisfatto \(u\) in \(S\):
4.          \(s\sb{u} = -s\sb{u}\)  # Inverti lo stato di \(u\)
5.      \textbf{return} \(S\)
\end{alltt}
\hrule 
\vspace{1\baselineskip}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\textwidth]{immagini/LocalSearch/state_flipping.png}
    \caption{Le parti (a)-(f) mostrano i passi di un'esecuzione dell'algoritmo State-Flipping per una rete di Hopfield a cinque nodi, terminando in una configurazione stabile (i nodi sono colorati in nero o bianco per indicare il loro stato). }
    \label{fig:state_flipping}
\end{figure}


\subsection{Convergenza e Complessità}
L'algoritmo State-flipping termina sempre con una configurazione stabile dopo al più $W = \sum_{e} |w_e|$ iterazioni.
...
Chiaramente, il valore minimo di questa funzione è 0 e il massimo è $W$.
Dobbiamo mostrare che $\Phi(S)$ aumenta di almeno 1 ad ogni flip:
\begin{itemize}
    \item Quando un nodo insoddisfatto $u$ cambia stato:
    \begin{itemize}
        \item Tutti gli archi "buoni" incidenti a $u$ diventano "cattivi".
        \item Tutti gli archi "cattivi" incidenti a $u$ diventano "buoni".
        \item Gli archi non incidenti a $u$ non cambiano.
    \end{itemize}
    \item Poiché $u$ era insoddisfatto, per definizione il peso degli archi cattivi incidenti era maggiore di quelli buoni.
    \item Di conseguenza, scambiando i due insiemi, il guadagno in termini di peso degli archi buoni supera la perdita:
    $$ \Phi(S') \ge 1 + \Phi(S) $$
\end{itemize}
Poiché la funzione $\Phi$ è limitata superiormente e cresce strettamente ad ogni passo (cioè fino a quando ci sono nodi insoddisfatti da flippare), l'algoritmo deve terminare al più in un numero finito di passi $W$.
