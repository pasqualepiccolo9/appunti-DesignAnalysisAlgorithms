...
3.      \textbf{Let} a priority queue \(Q\) contain all the vertices of \(G\) 
            using the \(D\) labels as keys.
4.      \textbf{while} \(Q\) is not empty \textbf{do}
5.          // pull a new vertex \(u\) into the cloud
6.          \(u = \text{value returned by } Q.\text{remove\_min}()\)
7.          \textbf{for each} vertex \(v\) adjacent to \(u\) such that \(v\) in \(Q\) \textbf{do}
8.              // perform the relaxation procedure on edge \((u,v)\)
9.              \textbf{if} \(D[u] + w(u,v) < D[v]\) \textbf{then}
10.                 \(D[v] = D[u] + w(u,v)\)
11.                 Change to \(D[v]\) the key of vertex \(v\) in \(Q\).
12.     \textbf{return} the label \(D[v]\) of each vertex \(v\)                                  
\end{alltt}
\hrule 
\vspace{1\baselineskip}
...
def shortest_path_lengths(g, src):
    ...
    """
    # MAPPA: Vertice (oggetto) -> Distanza stimata (numero)
    d = {}

    # MAPPA: Vertice (oggetto) -> Distanza definitiva (numero)
    cloud = {}

    # CODA CON PRIORITA': Priorita'=Distanza (d[v]), Elemento=Vertice (v)
    pq = AdaptableHeapPriorityQueue()

    # MAPPA: Vertice (oggetto) -> Locator (oggetto handle)
    pqlocator = {}
...
2.      \textbf{for all} \(v\) in \(G\).\text{vertices}() \textbf{do}
3.          \textbf{if} \(v = s\) \textbf{then}
4.              \(d(v) = 0\)
5.          \textbf{else}
6.              \(d(v) = \infty\)
7.      \textbf{for} \(i = 1\) \textbf{to} \(n - 1\) \textbf{do}
8.          \textbf{for each} \(e\) in \(G\).\text{edges}() \textbf{do}
9.              // relax edge \(e\)
10.             \(u = G\).\text{origin}(\(e\))
11.             \(v = G\).\text{opposite}(\(u, e\))
12.             \(r = d(u) + \text{weight}(e)\)
13.             \textbf{if} \(r < d(v)\) \textbf{then}
14.                 \(d(v) = r\)
\end{alltt}
\hrule 
\vspace{1\baselineskip}
...
8.      \textbf{for} \(u = 1\) \textbf{to} \(n - 1\) \textbf{do} // in topological order
9.          \textbf{for each} \(e\) in \(G\).\text{outEdges}(\(u\)) \textbf{do}
10.             // relax edge \(e\)
11.             \(v = G\).\text{opposite}(\(u, e\))
12.             \(r = d(u) + \text{weight}(e)\)
13.             \textbf{if} \(r < d(v)\) \textbf{then}
