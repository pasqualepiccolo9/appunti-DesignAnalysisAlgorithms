...
\subsection*{Soluzione}
\begin{itemize}
    \item Costruiamo un \textbf{Suffix Tree generalizzato}\footnote{
        Un \emph{Suffix Tree generalizzato} rappresenta tutti i suffissi di un insieme di stringhe, anziché di una singola stringa; Nota: è stato utilizzato nelle soluzioni dei paragrafi precedenti ai problemi proposti.
        } $T$ per le $K$ stringhe.
    \begin{itemize}
        ...
        \begin{itemize}
            \item Questa operazione è il \textbf{collo di bottiglia della soluzione}\footnote{
            La soluzione proposta viene eseguita in tempo $O(K \cdot m)$, ma esiste una soluzione più avanzata in grado di raggiungere tempo lineare $O(n)$.
        } e può essere eseguita in tempo $O(K \cdot m)$.
        \end{itemize}
